...
@ContextConfiguration(initializers = TextContentServiceTest.Initializer.class,
    classes = TestLocalstackS3Application.class)
public class TextContentServiceTest {
  ...
  public void shouldCreateTextContent() {
    ...
    assertThat(created).extracting("key", "text")
        .contains(textContent.getKey(), textContent.getText());

    Optional<TextContent> found = textContentService.findTextContentByKey(textContent.getKey());
    assertThat(found).isPresent();
    assertThat(found.get()).extracting("key", "text")
        .contains(textContent.getKey(), textContent.getText());
  }
  ...
  static class Initializer implements
    ...
    public void initialize(ConfigurableApplicationContext configurableApplicationContext) {
      TestPropertyValues values = TestPropertyValues.of(
          "aws.credentials.accessKey=" + localStackContainer.getDefaultCredentialsProvider()
              .getCredentials().getAWSAccessKeyId(),
          "aws.credentials.secretKey=" + localStackContainer.getDefaultCredentialsProvider()
              .getCredentials().getAWSSecretKey(),
          "aws.region=us-east-1",
          "aws.endpoint=http://" + localStackContainer.getContainerIpAddress() + ":"
              + localStackContainer.getMappedPort(4572),
